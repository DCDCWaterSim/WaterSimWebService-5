...
namespace WaterSimWebServiceVersion2
{
    ...
    [WebService(Namespace = "http://dasampson.net/")]
    ...
    public class WaterSimDCDCModel : System.Web.Services.WebService
    {
        ...
        // version
        string WebServiceVersion = "5.0 8/31/16";
            ...
            lock (GetWSimThreadLock)
            {
                ...
                if (Application[WaterSimFlag].ToString() != WSCREATED)
                {
                    try
                    {
                        ...
                        //TheModelType = WSVersiontemp.Substring(0, 3).ToUpper(); DAS on 05.12.17
                        TheModelType = WSVersiontemp.Substring(0, 3).ToUpper();
                        string ThePhoenixModel = WSVersiontemp.Substring(4, 1).ToUpper();
        ...
        public string WaterSimVersion()
        {
            ...
            try
            {
                ...
                if (MyWSim != null)
                {
                    response = "WebService: "+WebServiceVersion+ " API:" + MyWSim.API_Version + " M:" + MyWSim.Model_Version;
                }
                else
                {
                    response = ErrMessage;
                ...
                if (MyWSim != null)
                {
                    ...
                    if (Application[WaterSimStatus].ToString() != WSBUSY)
                        ...
                        {   // we are busy
                            ...
                            //{
                            //    MyWSim.ResetScenarioData(WSU);
                            //}
                            //// ==============================================================

                            // Run all years, using defaults for values not set
                            MyWSim.Simulation_AllYears();
                            ProviderIntArray PV = new ProviderIntArray(0);
                            PV = MyWSim.Colorado_Annual_Deliveries.getvalues();
                            // Stop
...
///        
        ...
        #endregion
        // ====================================================================
        // Load (reload) extended Documentation file (DocumentationItems.txt)
        //=====================================================
        // 10.12.17 das
        [WebMethod]
        public string ReloadExtendDocFile()
        {
            string response = "";
            string ErrMessage = "";
            try
            {
                ...
                if (MyWSim != null)
                {
                    MyWSim.ParamManager.Extended.LoadExternalDocumentation(MyWSim.DataDirectory);
                }
                else
                {
                    response = ErrMessage;
            ...
            if (response == "")
            {
                response = "Extended Documentation Was Reloaded Successfully";
            }
            string EventString = response;
            string UIP = HttpContext.Current.Request.UserHostAddress;

            TheLog.writeToLog(",[RELOADEXD],[" + UIP + "]," + EventString);
            TheLog.Dispose();
